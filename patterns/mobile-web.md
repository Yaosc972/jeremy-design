...
- **聚焦的输入框必须可见**：聚焦后被键盘遮住是硬伤——在 `focus`/`visualViewport.resize` 后先判断是否真被遮挡（输入框 `getBoundingClientRect()` 与 visual viewport 在 layout viewport 中的区间 `[offsetTop, offsetTop + height]` 比较，避免忽略缩放/平移后的偏移），被遮再 `scrollIntoView({block:'center'})`；未被遮不动滚动位置，避免每次聚焦都跳屏。
...
- 弹层是否占用返回键，先做设计决定：构成**可导航状态**（按返回键应关闭、可深链接、刷新后能恢复）的模态/抽屉与项目路由器统一管理历史（`pushState` 与 `back()` 成对协调）；普通确认框、提示层不默认增加历史——返回键直接退页面对它们同样合理。
- 多步流程（向导、结账）：前进到可导航步骤时创建记录，后退复用路由器/history 返回，不在后退时再 push 一条记录；返回键行为与界面后退按钮一致。
...
- 视口高度**按目的选择**，不要一律 `100dvh`：全屏应用壳/整屏滚动容器用 `100dvh`（动态视口，随地址栏收展）；内容页让内容自然撑高即可；关键首屏元素用 `min-height` 兜底或用 `svh`（最小视口）做评估基准。注意 `dvh` 在滚动中会持续改变尺寸，滚动过程依赖它的布局有性能代价（W3C 提示）。不要用裸 `100vh`——地址栏收展时它比可视区高，底部内容被裁。
...
先用设备模拟检查布局和基本交互，再在目标真机完整走一遍软键盘与系统返回行为；设备模拟不能证明软键盘避让通过（比截一张静态图有价值得多）：

> **打开表单 → 聚焦靠近屏幕底部的输入框 → 输入 → 触发并查看错误信息 → 修正 → 完成提交**

过程中检查：输入框是否被键盘遮挡、底部按钮是否可及、错误信息是否可见、提交反馈是否可达、返回键行为是否正确。按 `detail-audit.md` §0 检查相关窄视口，检测器作为可选辅助。

坐标参考：[MDN VisualViewport.offsetTop](https://developer.mozilla.org/en-US/docs/Web/API/VisualViewport/offsetTop)。

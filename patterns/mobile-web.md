...
- **聚焦的输入框必须可见**：聚焦后被键盘遮住是硬伤——在 `focus`/`visualViewport.resize` 后先判断是否真被遮挡（输入框 `getBoundingClientRect().bottom` 与 `visualViewport.height` 比较），被遮再 `scrollIntoView({block:'center'})`；未被遮不动滚动位置，避免每次聚焦都跳屏。

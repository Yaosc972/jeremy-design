...
- 长列表（数千行）用虚拟滚动库，不要手写；同时确认吸顶表头与键盘焦点在虚拟化下的行为（`patterns/workbench.md` §4）。
- 行级操作的事件处理避免整个列表重渲染：行组件做记忆化（`memo`）或把 handler 提到稳定引用。
...
按 `detail-audit.md` 跑矩阵。React 项目的特有检查项：StrictMode 下无双份副作用（§3）、暗色切换不残留过期状态、列表增删后 key 行为符合预期。

...
| **视觉重构验收**（改既有页面视觉时强制） | `detail-audit.md` §四 + `scripts/visual-review.mjs`（before/after 对照片） |
...
0. **执行流程**（每次任务走一遍）：按 `design-workflow.md` 先定方向（谁使用、核心任务、真实内容、已有设计系统）→ 判断**任务类型**（新建 / 改版 / 局部修复 / 参考复刻——局部修复优先保留原结构，参考复刻优先忠实参考，只有新建或明确改版才主动建立整套视觉方案）与**产品类型**（内容展示网站 / 工作台 / 移动 Web）→ 确认运行环境与输入方式（原生 or Web、目标浏览器、鼠标/触控/键盘）→ 按路由表读任务相关 reference → 区分来源标记，不混用原生 point 与 CSS 单位 → 实现正常、交互、异常三态（按 `states-and-forms.md` 展开）→ 验证无障碍（对比度、字号缩放、reduced-motion、键盘/读屏）、布局与动效（动画中断不跳变）→ **交付前细节审查**（`scripts/audit-matrix.sh` 按实际状态矩阵运行；改既有页面视觉时另跑 `scripts/visual-review.mjs` 对照片并逐行勾选，见 `detail-audit.md` §四），验收分三级：**阻断问题清零**（关键内容不可读、主要操作不可达、布局严重溢出、运行错误）、**警告逐项复核并记录处置**、**已知例外备案**——阻断清零即通过，不以消除所有启发式告警为目标修改正常布局；修复优先解决布局根因，再复跑矩阵确认其他状态没有退化 → 向用户报告**已验证项与未验证项**；未经实际验证，不声称实现"已符合 HIG"（详细口径见 `detail-audit.md`）
